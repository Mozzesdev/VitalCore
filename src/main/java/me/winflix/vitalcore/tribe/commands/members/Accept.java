...
public class Accept extends SubCommand {
    ...
    public void perform(Player sender, String[] args) {
        ...
        Tribe tribeToLeave = senderUser.getTribe();
        ...
        if (tribeToLeave.getMembers().size() > 1) {
            if (senderMember.getRange().getName().equals(RankManager.OWNER_RANK.getName())) {
                ...
                tribeToLeave.replaceMember(UUID.fromString(newOwner.getId()), newOwner);
                tribeToLeave.setTribeName("Tribe_of_" + newOwner.getPlayerName());
            }
            tribeToLeave.removeMember(senderMember);
            TribesCollection.saveTribe(tribeToLeave);
        } else {
            // Si la tribu solo tiene al jugador actual, eliminar la tribu
            TribesCollection.deleteTribe(tribeToLeave);
        }

        // Establecer el rango del miembro receptor y agregarlo a la tribu a unirse
        senderMember.setRange(RankManager.MEMBER_RANK);
        tribeToJoin.addMember(senderMember);
        tribeToJoin.removeInvitation(invitation.get());
