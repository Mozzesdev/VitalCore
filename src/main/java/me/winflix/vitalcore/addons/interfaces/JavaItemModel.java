...
public class JavaItemModel {
    ...
    public int scaleToFit() {
        ...
        final int size = (int) Math.ceil(this.maxDistToOrigin * 0.041666668f);
        final float scale = 1.0f / size;
        for (final JavaElement element : this.elements) {
            final float[] origin = (float[]) ((element.getRotation() == null) ? null : element.getRotation().origin);
            for (int i = 0; i < 3; ++i) {
                element.from[i] = MathUtils.clamp((element.from[i] - 8.0f) * scale + 8.0f, -16.0f, 32.0f);
                element.to[i] = MathUtils.clamp((element.to[i] - 8.0f) * scale + 8.0f, -16.0f, 32.0f);
                if (origin != null) {
