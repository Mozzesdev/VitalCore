...
import me.winflix.vitalcore.addons.interfaces.ProcessedCube.UV;
import me.winflix.vitalcore.addons.interfaces.ItemGroup; // Usa double
...
import me.winflix.vitalcore.addons.interfaces.ProcessedCube; // Usa double
import me.winflix.vitalcore.addons.interfaces.ProcessedPackData;
import me.winflix.vitalcore.addons.solvers.RotationSolver;
...
public class ModelProcessor {
    ...
    public static ProcessedBbModel process(BbModel bbModel) {
        ...
        VitalCore.Log.info("[ModelProcessor] Procesando modelo: " + bbModel.getName());
        ...
        for (Bone bone : allBonesInHierarchy) {
            ...
            if (directCubes != null && !directCubes.isEmpty()) {
                ...
                for (BbModel.Cube cube : directCubes) {
                    ...
                    Vector3d cubePivot = MathUtils.toVector3d(cube.getPivot()).sub((Vector3fc) proBone.getInitialPivot());
                    ...
                    if (!modelsForThisBone.isEmpty()) {
                        ...
                        for (JavaItemModel currentJavaModel : modelsForThisBone) {
                            ...
                            if (modelsForThisBone.size() == 1) {
                                geometryId = ResourcePackManager.VITALCORE_NAMESPACE + ":models/" + bbModel.getName()
                                        + "/" + bonePartName;
                                itemId = ResourcePackManager.VITALCORE_NAMESPACE + ":items/" + bbModel.getName() + "/"
                                        + bonePartName;
        ...
        VitalCore.Log.info("[ModelProcessor] Modelo '" + bbModel.getName() + "' procesado. Total huesos procesados: "
                + processedBonesMap.size() + ", Raíces: " + rootProcessedBones.size());
        return proModel;
    }
    ...
    private static String generateModelId(BbModel bbModel, Bone bone, int partIndex) {
        String namespace = ResourcePackManager.VITALCORE_NAMESPACE;
        String modelNameClean = bbModel.getName().toLowerCase().replaceAll("[^a-z0-9/._-]", "_");
        String boneNameClean = bone.getName().toLowerCase().replaceAll("[^a-z0-9/._-]", "_");
        String basePath = namespace + ":models/" + modelNameClean + "/" + boneNameClean;
        return (partIndex >= 0) ? basePath + "_part" + partIndex : basePath;
    }
