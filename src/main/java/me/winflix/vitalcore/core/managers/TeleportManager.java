...
public class TeleportManager {
    ...
    public static void sendTeleportRequest(Player sender, Player target) {
        if (target.getUniqueId().equals(sender.getUniqueId())) {
            Utils.errorMessage(target, "No puedes enviarte una solicitud de teletransporte a ti mismo.");
            return;
        }
    ...
    public static void acceptRequest(Player target, String senderName) {
        ...
        Utils.infoMessage(sender, "Teleportación en " + cooldownSeconds + " segundos...");

        BukkitTask tornadoTask = Bukkit.getScheduler().runTaskTimer(VitalCore.getPlugin(), () -> {
            spawnTornadoEffect(sender);
        }, 0L, 2L);
        ...
        cooldownTaskHolder[0] = Bukkit.getScheduler().runTaskTimer(VitalCore.getPlugin(), () -> {
            if (counter[0] <= 0) {
                ...
                Utils.successMessage(target,
                        "Has aceptado la solicitud de teletransporte de " + sender.getName() + ".");
                ...
                if (counter[0] <= 3) {
                    Utils.infoMessage(sender, "Irás en &a" + counter[0] + "&7...");
                }
                counter[0]--;
            }
        }, 0L, 20L);
