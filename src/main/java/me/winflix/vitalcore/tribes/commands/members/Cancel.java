...
public class Cancel extends SubCommand {
    ...
    public void perform(Player p, String[] args) {
        ...
        Tribe targetTribe = TribesDAO.getTribeByName(tribeName);
        ...
        List<Invitation> playerInvitations = InvitationsManager.getPlayerInvitations(p.getUniqueId());
        Optional<Invitation> invitationOptional = playerInvitations.stream()
                .filter(inv -> inv.getType() == Invitation.InvitationType.TRIBE_TO_PLAYER)
                .filter(inv -> inv.getSenderId().equals(targetTribe.getId()))
                .findFirst();
